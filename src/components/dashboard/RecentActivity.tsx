            ...
            <>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="space-y-1">
                      <div className="flex items-center gap-2">
                        ...
                        <span
                          className={`text-tiny px-2 py-0.5 rounded-full font-medium ${getDifficultyColor(session.difficulty)}`}
                        >
                    ...
                    <div className="flex items-center space-x-4">
                      {session.overallScore && (
                        <div className="text-right">
                          <div
                            className={`px-2 py-1 rounded-small bg-gradient-to-r ${getScoreColor(session.overallScore)} text-white font-medium text-tiny transition-all duration-300`}
                          >
                            Score: {session.overallScore}% -{" "}
                            {getScoreText(session.overallScore)}
                          </div>
                        </div>
                      )}
                    </div>
              ...
              {sortedSessions.length > 3 && (
                ...
                >
                  {showAll
                    ? "Show Less"
                    : `Show ${sortedSessions.length - 3} More`}
                </button>
              )}
            </>
          )}
