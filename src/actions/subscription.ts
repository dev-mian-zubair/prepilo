...
export const subscription = {
  ...
  async addMinutes(minutes: number): Promise<SubscriptionStatus | null> {
    const user = await getUser();
    if (!user) return null;
  ...
  async deductMinutes(minutes: number): Promise<boolean> {
    const user = await getUser();
  ...
  async checkAvailableMinutes(minutes: number): Promise<boolean> {
    const user = await getUser();
    if (!user) return false;
